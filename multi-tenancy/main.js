...
const MONKEYS_ADMIN_DIST_FOLDER = process.env.MONKEYS_ADMIN_DIST_FOLDER || 'dist';

// Read configuration from `config.yaml`
let configFilePath = path.join(__dirname, './config.yaml');
if (fs.existsSync(configFilePath)) {
  configFilePath = configFilePath;
} else if (fs.existsSync('/etc/monkeys-admin/config.yaml')) {
  configFilePath = '/etc/monkeys-admin/config.yaml';
} else {
...
const startServers = async () => {
  ...
  for (let i = 0; i < serverConfigYamls.length; i++) {
    ...
    // Run migration
    const migrationResult = await runCommand(MONKEYS_ADMIN_DIST_FOLDER, `yarn migration:run`, {
      MONKEYS_CONFIG_FILE: configFile,
    });
    logger.info(`Migration result for app ${i}: ${migrationResult}`);

    // Start server
    const script = `${MONKEYS_ADMIN_DIST_FOLDER}/main`;
